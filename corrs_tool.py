...
class App:
    def __init__(self,master,imgList,camList):
        ...
        maxNI, maxNJ = 1280.0, 720.0
        self.reduceFactor = max(firstImg.size[0]/maxNI, firstImg.size[1]/maxNJ)
        ...
        for j in range(2):
          ...
          frame1.grid(row=0, column=j+1, sticky=NW)      
          
          currFrame = (len(self.imgList) / self.numImageFrames) * frCount
          imageFrame = ImageFrame(frame1, label, labString, currFrame, 0, j);
    ...
    def point_from_rays(self):
      ...
      uvs = []
      cams = []
      for imgIdx, scaled in self.UVMap.items():
        uv = numpy.multiply(scaled,self.reduceFactor)
        uvs.append(uv)
        cam = load_perspective_camera(self.camList[imgIdx])
        cams.append(cam)
    ...
    def runprobe(self,event,iFrame):
        ...
        self.UVMap[iFrame.currImg] = (event.x, event.y)
